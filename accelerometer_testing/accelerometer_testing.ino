...
void loop()
{
 ...
 //////////////////////////////////////////////////////////////////////////////////////
  ...
  while (Serial.available()>0){ sizeBuffer = Serial.readBytesUntil('\n', inBuffer, BUFF_MAX); }

  // if the file is available, write to it:
  int dX = Serial.read();
  if (dataFile) {
   digitalWrite(LED, HIGH); 
   // dataFile.println(accel_t_gyro.value.x_accel + ',' + accel_t_gyro.value.y_accel + ',' + accel_t_gyro.value.z_accel+',');
    dataFile.print(accel_t_gyro.value.x_accel, DEC);
    dataFile.print(F(", "));
    dataFile.print(accel_t_gyro.value.y_accel, DEC);
    dataFile.print(F(", "));
    dataFile.print(accel_t_gyro.value.z_accel, DEC);
    dataFile.print(F(", "));
    dataFile.print("Test");
    dataFile.println(F(""));
    ...
    //Serial.println(accel_t_gyro.value.x_accel + ',' + accel_t_gyro.value.y_accel + ',' + accel_t_gyro.value.z_accel+',');
    Serial.print(accel_t_gyro.value.x_accel, DEC);
    Serial.print(F(", "));
    Serial.print(accel_t_gyro.value.y_accel, DEC);
    Serial.print(F(", "));
    ...
    Serial.write(outBuffer, sizeBuffer);
    Serial.print("\r\n");
    

    
   sizeBuffer = Serial.readBytesUntil('\n', inBuffer, BUFF_MAX);  // read one 
   Serial.write(outBuffer, sizeBuffer);
